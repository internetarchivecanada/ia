...
/// Run the CLI: parse argv, load config, and dispatch to the chosen command.
///
/// This is the whole program. `main` is a thin wrapper so the CLI can also be
/// driven in-process — integration tests can call this instead of spawning the
/// binary, which is faster and yields real backtraces.
///
/// Restores the default `SIGPIPE` disposition as its first act, so piping
/// output into a command that exits early terminates quietly instead of
/// panicking.
