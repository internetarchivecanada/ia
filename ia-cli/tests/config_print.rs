...
fn print_cookies_json() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    let parsed: serde_json::Value = serde_json::from_str(&stdout).unwrap();
    assert_eq!(parsed["logged-in-user"], "user%40example.com");
    assert_eq!(parsed["logged-in-sig"], "test-sig");
}
...
#[test]
fn print_cookies_no_cookies_fails() {
    let dir = tempfile::tempdir().unwrap();
    let ini_path = dir.path().join("ia.ini");
    let mut f = std::fs::File::create(&ini_path).unwrap();
    writeln!(f, "[general]").unwrap();
...
#[test]
fn whoami_no_credentials_fails() {
    let dir = tempfile::tempdir().unwrap();
    let ini_path = dir.path().join("ia.ini");
    let mut f = std::fs::File::create(&ini_path).unwrap();
    writeln!(f, "[general]").unwrap();
